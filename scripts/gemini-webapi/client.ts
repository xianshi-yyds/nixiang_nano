...
function is_generated_image_url(s: string): boolean {
  return /^https:\/\/lh3\.googleusercontent\.com\//.test(s);
}

function collect_generated_image_urls_from_text(text: string, limit: number = 8): string[] {
  const seen = new Set<string>();
  const out: string[] = [];
  const matches = text.match(/https:\/\/lh3\.googleusercontent\.com\/[^\s"'\\<>()]+/g) ?? [];
  for (const raw of matches) {
    const url = raw.replace(/[),.\]}]+$/, '');
    if (!is_generated_image_url(url) || seen.has(url)) continue;
    seen.add(url);
    out.push(url);
...
function find_body_json(response_json: unknown): { body_json: unknown[]; body_index: number } | null {
  if (!Array.isArray(response_json)) return null;
  for (let part_index = 0; part_index < response_json.length; part_index++) {
    ...
    try {
      ...
      if (get_nested_value(part_json, [4], null)) {
        return { body_json: part_json, body_index: part_index };
      }
    } catch {}
  }
  return null;
...
function parse_output_from_response_json(
  response_json: unknown,
  opts: {
    proxy: string | null;
    cookies: Record<string, string>;
    allowPartialGenerated?: boolean;
  },
): ModelOutput {
  const found = find_body_json(response_json);
  if (!found) {
    throw new Error('No body');
  }

  const { body_json, body_index } = found;
  ...
  for (let candidate_index = 0; candidate_index < candidate_list.length; candidate_index++) {
    ...
    for (const w of get_nested_value<unknown[]>(candidate, [12, 1], [])) {
      ...
      if (!url) continue;
      web_images.push(new WebImage(url, String(get_nested_value(w, [7, 0], '')), String(get_nested_value(w, [0, 4], '')), opts.proxy));
    }
      ...
      if (Array.isArray(response_json)) {
        for (let part_index = body_index; part_index < response_json.length; part_index++) {
          ...
          try {
            ...
            const urls = collect_strings(cand, is_generated_image_url, 1);
            if (urls.length > 0) {
              img_body = part_json;
              break;
            }
      ...
      if (img_body) {
        ...
        for (let img_index = 0; img_index < gen.length; img_index++) {
          ...
          const alt =
            ...
            '';
          generated_images.push(new GeneratedImage(url, title, alt, opts.proxy, opts.cookies));
        }

        if (generated_images.length === 0) {
          const urls = collect_strings(img_candidate, is_generated_image_url, 4);
          for (const url of urls) {
            generated_images.push(new GeneratedImage(url, '[Generated Image]', '', opts.proxy, opts.cookies));
          }
        }
      } else if (!opts.allowPartialGenerated) {
  ...
  return new ModelOutput({ metadata, candidates: out });
}
  ...
  async generate_content(
    ...
    return await this._run(async () => {
      ...
      if (res.body) {
        ...
        const chunkLogPath = '/tmp/gemini-web-image-stream.log';
        let chunkIndex = 0;
        while (true) {
          ...
          if (files?.length) {
            await appendFile(
              chunkLogPath,
              `\n--- chunk ${chunkIndex} bytes=${value.byteLength} ---\n${txt.slice(-2000)}\n`,
              'utf8',
            ).catch(() => {});
          }
          if (files?.length) {
            const rawUrls = collect_generated_image_urls_from_text(txt, 4);
            if (rawUrls.length > 0) {
              const early = new ModelOutput({
                metadata: chat?.metadata ?? [],
                candidates: [
                  new Candidate({
                    rcid: 'stream-generated',
                    text: '',
                    thoughts: null,
                    web_images: [],
                    generated_images: rawUrls.map((url, idx) => new GeneratedImage(url, `[Generated Image ${idx + 1}]`, '', this.proxy, this.cookies)),
                  }),
                ],
              });
              if (chat instanceof ChatSession) chat.last_output = early;
              return early;
            }
          }
          try {
            response_json = extract_json_from_response(txt);
            const early = parse_output_from_response_json(response_json, {
              proxy: this.proxy,
              cookies: this.cookies,
              allowPartialGenerated: true,
            });
            if (early.candidates.some((c) => c.images.some((img) => img instanceof GeneratedImage))) {
              if (chat instanceof ChatSession) chat.last_output = early;
              return early;
            }
          } catch {}
      ...
      try {
        if (!find_body_json(response_json)) throw new Error('No body');
      } catch {
        await this.close();
        try {
      ...
      try {
        const output = parse_output_from_response_json(response_json, {
          proxy: this.proxy,
          cookies: this.cookies,
        });
